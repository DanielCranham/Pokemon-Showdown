/**
* Wi-Fi chat-plugin.
* Handles giveaways in the following formats:
*	- question
*	- lottery
**/
...
var QuestionGiveAway = (function () {
	...
	QuestionGiveAway.prototype.guessAnswer = function (user, guess, output) {
		...
		if (!this.answered[userid])
			this.answered[userid] = 0;
	...
	QuestionGiveAway.prototype.change = function (key, value, user, output) {
		if (user.userid !== this.host.userid)
			return this.sendReply("You cannot edit this giveaway. Only the host/starter can edit.");
		if (this.phase !== 'pending')
			return output.sendReply("The giveaway has started, and the question/answer cannot be changed.");
		...
		case 'answer':
			...
			if (Object.keys(ans).length <= 0)
				return output.sendReply("You must specify atleast one answer. Note: answers can be utmost 3 words long, and cannot contain any special characters.");
...
var LotteryGiveAway = (function () {
	function LotteryGiveAway(host, giver, room, options) {
		...
		this.reminder = '<center><div class = "broadcast-blue"><font size = 3><b>It\'s giveaway time!</b></font><br/>' +
			...
			'<font size = 1><b><u>Note:</u> Please do not join if you don\'t have a 3DS and a copy of PokÃ©mon X, Y, Î©R, or Î±S';
	...
	LotteryGiveAway.prototype.onEnd = function (force) {
		...
		finallist = Tools.escapeHTML(finallist.join(', '));
		this.room.addRaw('<div class = "broadcast-blue"><font size = 2><b>Lottery Draw:</b></font><br/>' +
			'Our lucky winners are <b>' + finallist + '!</b> Congratulations!');

		for (var id in this.winners) {
			var targetUser = this.winners[id];
			if (targetUser && targetUser.connected)
				targetUser.send('|popup|You have won the lottery giveaway! PM **' + this.giver.name + '** to claim your prize!');
		}
		this.room.update();
		delete giveaways[this.room.id];
		return;
...
var commands = {
	...
	qg: function (target, room, user) {
		...
		if (!this.can('giveaway', null, room)) return false;
		...
		if (Object.keys(options.answers).length <= 0)
			return this.sendReply("You must specify atleast one answer. Note: answers can be utmost 3 words long, and cannot contain any special characters.");
	...
	endgiveaway: 'end',
		...
		var giveaway = giveaways[room.id];
		if (!giveaway) return this.sendReply('There is no giveaway going on at the moment.');
		if (!this.can('giveaway', giveaway.host, room) && user.userid !== giveaway.host.userid) return false;
	...
	remind: function (target, room, user) {
		...
		case 'lottery':
			if (!this.canBroadcast()) return;
			this.sendReply(giveaway.reminder);
			break;
		}
	},
	'': 'help',
	help: function (target, room, user) {
		...
		this.sendReplyBox(
			...
			'- remind - shows the details of a giveaway.<br />' +
